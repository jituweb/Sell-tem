...
class NodeController extends AdminController
{
    ...
    public function add($request, $response, $args): ResponseInterface
    {
        ...
        $node->server_client    = trim($request->getParam('server_client')) ?: null;  // 新增这一行
    ...
    public function ajax($request, $response, $args): ResponseInterface
    {
        ...
        if ($search) {
            ...
            $query->where('id', 'LIKE', "%$search%")
                ...
                ->orwhere('server_client', 'LIKE', "%$search%")  // 添加这一行
        ...
        foreach ($nodes as $node) {
            ...
            $tempdata['server_client']              = $node->server_client;  // 添加这一行
