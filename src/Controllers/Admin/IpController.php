...
class IpController extends AdminController
{
    ...
    public function alive($request, $response, $args)
    {
       $table_config['total_column'] = array(
    'id' => 'ID', 'userid' => '用户ID',
    'user_name' => '用户名', 'user_level' => '用户等级',
    'class_expire' => '等级过期时间',
    'nodeid' => '节点ID', 'node_name' => '节点名',
    'ip' => 'IP', 'location' => '归属地',
    'datetime' => '时间', 'is_node' => '是否为中转连接'
);
    ...
    'Select alive_ip.id,
            alive_ip.userid,
            user.user_name,
            user.class as user_level,
            user.class_expire,
            alive_ip.nodeid,
            ss_node.name as node_name,
            alive_ip.ip,
            alive_ip.ip as location,
            alive_ip.datetime,
            alive_ip.id as is_node
     from alive_ip,user,ss_node
     WHERE alive_ip.userid = user.id
       and alive_ip.nodeid = ss_node.id
       and `datetime` > UNIX_TIMESTAMP() - 60'
);
...
$datatables->edit('user_level', static function ($data) {
    return $data['user_level'] > 0 ? "VIP".$data['user_level'] : "普通用户";
});

$datatables->edit('class_expire', static function ($data) {
    $expire = $data['class_expire'];

    if ($expire === null || $expire == '1970-01-01') {
        return '永久';
    }
