...
class URL
{
    ...
    */
    ...
    public static function getNew_AllItems(User $user, array $Rule): array
    {
        ...
        // 根据客户端类型替换域名
        $isOfficialClient = isset($Rule['official_client']) ? $Rule['official_client'] : false;
        $nodes = self::replaceNodesServer($nodes, $isOfficialClient);
        ...
        if ($is_mu != 0 && in_array($Rule['type'], ['all', 'ss', 'ssr'])) {
            ...
            // 单端口节点也需要替换域名
            $mu_nodes = self::replaceNodesServer($mu_nodes, $isOfficialClient);
        }
     ...
     * @param bool $isOfficialClient 是否为官方客户端
     * @return Node
     */
    public static function replaceNodeServer(Node $node, bool $isOfficialClient = false): Node
    {
        // 如果是官方客户端且配置了专用域名，则替换
        if ($isOfficialClient && !empty($node->server_client)) {
            $clonedNode = clone $node;
            $clonedNode->server = $node->server_client;
            return $clonedNode;
        }
        return $node;
    }
     ...
     * 批量替换节点域名
     * 
     * @param \Illuminate\Database\Eloquent\Collection $nodes 节点集合
     * @param bool $isOfficialClient 是否为官方客户端
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public static function replaceNodesServer($nodes, bool $isOfficialClient = false)
    {
        if (!$isOfficialClient) {
            return $nodes;
        }
        
        return $nodes->map(function ($node) use ($isOfficialClient) {
            return self::replaceNodeServer($node, $isOfficialClient);
        });
    }
}
