    ...
    <div class="card">
        <div class="card-main">
            <div class="card-inner">
                <h5>活跃转化统计</h5>
                <br>
                <font color="#B94FFF">复购转化池</font><br>
                <p id="conversion_stats_text" style="line-height: 1.8; margin-top: 10px;">loading</p>
            </div>
        </div>
    </div>
</div>
...
function getConversionStats() {
        console.log('开始请求转化统计数据...');
        
        $.ajax({
            type: "GET",
            url: "/admin/api/analytics/conversion_stats",
            dataType: "json",
            timeout: 10000,
            success: function (data) {
                ...
                if (data.success) {
                    ...
                    var html_text = `
                        2 天过期用户数：${statsData.expired_2day || 0} 人 <br>
                        2-7 天过期用户数：${statsData.expired_2to7day || 0} 人<br>
                        7-30 天订阅用户数：${statsData.expired_7to30day || 0} 人 <br>
                        30-93 天订阅用户数：${statsData.expired_30to93day || 0} 人 <br>
                        93-183 天订阅用户数：${statsData.expired_93to183day || 0} 人<br>
                        183-365 天订阅用户数：${statsData.expired_183to365day || 0} 人 <br>
                        365天以上订阅用户数：${statsData.expired_365plus_day || 0} 人 <br>
                        <strong>总订阅用户数：${statsData.total_expired || 0} 人</strong><br>
                    `;
                    
                    $('#conversion_stats_text').html(html_text);
                } else {
                    console.error('API 返回错误:', data.error);
                    $('#conversion_stats_text').html('数据加载失败：' + (data.error || '未知错误'));
                }
            },
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                $('#conversion_stats_text').html('❌ ' + errorMsg);
            }
        });
    }
